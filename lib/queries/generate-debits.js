  ...
  return storage.models.StorageEvent
    .aggregate([
    {
      $match: {
        $and: [
          {success: true},
          {"userReport.exchangeResultMessage": {$eq: "SHARD_UPLOADED"}}
        ]
      }
    },
    {
      $project: {
        ...
        dateShardDeleted: {$ifNull: ["$storeEnd", {$add: [new Date(), 0]}]},
        shardSize: "$storage"
      }
    },
    {
    ...
    },
    {
      $project: {
        ...
        entryBillableAmount: {$multiply: ["$entrySizeGb", "$shardHoursOverlapWithBillingPeriod", gbHourPrice]}
      }
    },
    {
      $group: {
    ...
    .aggregate([
      {
        $match: {
          $and: [
            ...
            {success: true},
            {"userReport.exchangeResultMessage": {$eq: "SHARD_DOWNLOADED"}}
          ]
        }
      },
      {
