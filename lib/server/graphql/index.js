...
module.exports = {
  ...
  addPaymentProcessor: function(name, data) {
    ...
    return _models.PublicKey.findOne({_id: publicKeyHeader})
      .then((publicKey)=> {
        const user = publicKey.user;
        const paymentProcessor = new _models.PaymentProcessor({
          user: user,
          name: name,
  ...
  sendReferralEmail: function(senderEmail, recipientEmail) {
    console.log('mailer hit', senderEmail, recipientEmail);

    mailer.dispatch(recipientEmail, 'referral', {
      url: 'https://app.storj.io/signup',
  ...
  get currentUser() {
    ...
    return this.currentUserId
      .then((userId) => _models.User.findOne({ _id: userId }))
  ...
  get defaultPaymentProcessor() {
    ...
    return _models.PublicKey.findOne({_id: publicKeyHeader})
      .then((publicKey) => {
        return _models.PaymentProcessor.findOne({user: publicKey.user, default: true});
      });
  }
};
