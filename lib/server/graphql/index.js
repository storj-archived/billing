...
module.exports = {
  ...
  get defaultPaymentProcessor() {
    ...
    return this.models.PublicKey.aggregate([
      {
        $match: {
          _id: {$eq: publicKeyHeader}
        }
      },
      {
        $project: {
      ...
      {
        $match: {
          "paymentProcessor.default": {$eq: true}
        }
      }
    ]).then(paymentProcessors => paymentProcessors[0]);
  }
};
