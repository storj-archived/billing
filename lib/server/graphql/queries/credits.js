...
const CreditsQuery = {
  ...
  resolve: function(_, args) {
    ...
    // console.log("resolve models credits.js %j", Object.keys(models));
    const params = {};

    if (args.startDate && args.endDate) {
      params.created = {
        $gte: moment.utc(parseInt(args.startDate, 0)).add('1', 'month'),
        $lte: moment.utc(parseInt(args.endDate, 0)).add('1', 'month')
      };
    }
    ...
    return graphqlService.currentUser
        .then((user) => {
          ...
          // console.log("resolve models credit %j", Object.keys(models));
