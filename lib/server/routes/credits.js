...
function CreditsRouter(options) {
  ...
  this.listenForSignups();
  Router.apply(this, arguments);
}

inherits(CreditsRouter, Router);

CreditsRouter.prototype.listenForSignups = function () {
  const Marketing = this.models.Marketing;
  const options = {
    awaitData: true,
    numberOfRetries: 3
  }
  const filter = { "timestamp":{"$gte":Date.now()}};
  const Events = this.models.Event.find(filter).tailable(true, options).stream();

  Events
    .on('data', (event) => {
      console.log('ON DATA: ', event);
      if (event.name === 'userActivated') {
        console.log('USER ACTIVATED EVENT TRUE');
        //create marketing doc for both
        Marketing.create(event.data.email, function(err, marketing) {
         if (err) {
           return new errors.BadRequestError(err);
         }

         if (event.data.referralLink) {
           console.log('EVENT DATA REFERRAL LINK: ');
           _issueSignupCredit(event.data, 'referral')
             .then((credit) => {
               _convertReferralRecipient(credit, marketing, event.data);
             })
         } else {
           console.log('ISSUE REGULAR CREDIT');
           _issueSignupCredit(event.data);
         }
        })
      }
    })
...
CreditsRouter.prototype._convertReferralRecipient = function (credit, marketing, eventData) {
  const Referral = this.models.Referral;

  return new Promise((resolve, reject) => {
    Referral
      .findOne({
        referralLink: eventData.referralLink,
        recipient: {
      ...
      .then((referral) => {
        ...
        Referral
          .create(marketing, eventData.email, 'link')
