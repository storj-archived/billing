...
CreditsRouter.prototype.handleCreateCredit = function(req, res) {
  ...
  try {
     ...
     */
    this.models.PaymentProcessor.findByInvoice(customerId, 'stripe')
      .then(getUserCreditsAndDebits)
      .then(calculateAndApplyPromoCredit)
      .then(createAndApplyFreeThresholdCredit)
      .then(createCredit)
      .catch((err) => {
        console.error(err);
        throw new Error(err);
    ...
    console.error('Error caught in `CreditsRouter#createCredit`: %j', err);
