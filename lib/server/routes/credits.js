...
CreditsRouter.prototype.handleCreateAddress = function (req, res, next) {
  // handle creating a new wallet for a payment
  const proc = this.models.PaymentProcessor;

  // TODO: Add wallet code for addPaymentMethod();
  // proc.addPaymentMethod(req, res, next);
  return res.status(200);
};

CreditsRouter.prototype.handleTokenPayment = function (req, res, next) {
  // listen for specific webhook headers
  // setup webhook
};

CreditsRouter.prototype.checkTokenBalance = function (req, res, next) {
  // return balance of address from coinpayments
  const proc = this.models.PaymentProcessor;

  // TODO: create method on payment proc that checks token balances
  // const balances = proc.checkTokenBalance();

  return res.status(200);
};

CreditsRouter.prototype.handleRemoveAddress = function (req, res, next) {
  // remove a payment processor / address
  const proc = this.models.PaymentProcessor;

  // proc.removePaymentMethod();
  return res.status(200);
}

CreditsRouter.prototype.coinPaymentMiddleware = CoinPayments.ipn({
  merchantId: process.env.CP_MERCHANT_ID,
  merchantSecret: process.env.CP_SECRET_KEY
});

// coinpayment event handlers
coin.on('ipn_fail', function (data) {
...
coin.on('ipn_pending', function(data){
...
coin.on('ipn_complete', function(data){
  console.log("IPN COMPLETE");
  // update debits and credits to reflect
  const debit = this.models.Debit;
  const credit = this.models.Credit;
