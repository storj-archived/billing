...
CreditsRouter.prototype.handleCoinPayment = function(req, res, next) {
  coin.createTransaction({
    currency1: 'USD',
    currency2: 'STORJ',
    amount: req.body.amount,
    buyer_email: req.user._id
  }, (err, res) => {
    if (err) console.error('coinpayments api error: ', err);

    console.log('coinpayments transaction res: ', response);

    res.status(200).send(response);
  });
}

CreditsRouter.prototype.getCallbackAddress = function(req, res, next) {
  coin.getCallbackAddress('STORJ', process.env.CP_IPN_URL, (err, response) => {
    ...
    processor.save()
      ...
      .catch(error => {
        log.error(`Error saving CoinPayments processor: ${error}`);
        return next(new errors.InternalError(err.message));
      });
  })
}
...
CreditsRouter.prototype._definitions = function() {
  return [
    ...
    [
      'POST', '/credits/coinpayments',
      rawbody,
      this.authenticate,
      this.coinPaymentMiddleware,
      this.handleCoinPayment
    ],
    [
      'POST', '/credits/coinpayments/address',
      rawbody,
      this.authenticate,
