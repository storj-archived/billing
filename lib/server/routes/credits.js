    ...
    this.models.PaymentProcessor.findOne({
      ...
      .then((stripeProcessor) => {
        return Promise.all([
          ...
          Promise.resolve(stripeProcessor.user)
        ]);
      })
      .then((results) => {
        //-- lookup debits and credits for current user
        const [debits, credits, user] = results;
        ...
        if (promoBalance >= invoice.total) {
          ...
          invoiced_amount = (invoice.amount - promoBalance);
        }
        ...
        return new Promise((resolve, reject) => {
          const rejectWithError = (err) => {
            console.error('Error in CreditsRouter#createCredit: ' +
      ...
      .then(({ invoiced_amount, promo_amount, user }) => {
