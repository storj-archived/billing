...
PaymentProcessorsRouter.prototype.getWallets = function (req, res, next) {
  ...
  return Processor.findOne({
    ...
    .then((processor) => {
      console.log('processor found: ', processor);
      if (!processor) {
        return res.status(200).json('no wallets');
      }
  ...
  .then((processor) => {
    if (!processor) {
      return res.status(400)
        .send(errors.BadRequestError('no payment processor found'));
    }

    return processor.addPaymentMethod(req.body.currency)
      .then((res) => {
        console.log('added payment method with %s %s', req.body.currency, res);
