...
PaymentProcessorsRouter.prototype._addWallet = function(req, res) {
  ...
  .then((processor) => {
    ...
    return processor.addPaymentMethod(req.body.currency)
      .then((proc) => res.status(201).json(proc))
  })
  .catch((err) => res.status(500).send(errors.InternalError()));
}
...
PaymentProcessorsRouter.prototype.createWallets = function(req, res) {
  ...
  .then((processor) => {
    if (!processor) res.status(404).send(errors.BadRequestError('no processor found'));

    return processor.addPaymentMethod(req.body.currency)
      .then((data) => res.status(201).send(data))
      .catch((err) => {
        log.error('error adding payment method', err);
        return res.status(500).send(errors.InternalError('error adding payment method'));
      });
  });
}
