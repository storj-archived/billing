...
PaymentProcessorsRouter.prototype.handleIPN = (req, res) => {
  ...
  if (req.body.currency === 'STORJ') {
    User.findOne({
      ...
      if (err) log.error('error finding user by wallet', err);

      if (req.body.status === 0) {
         // search for credit
        this._findCryptoCredit(req.body)
         // if credit found, update status and invoiced_amount
      ...
      if (req.body.status === 100) {
        ...
        // if credit found, update credit and save it


        // if credit not found, create credit and save it
      }
...
PaymentProcessorsRouter.prototype._findCryptoCredit = (req, txn) => {
  const Credit = this.models.Credit;
