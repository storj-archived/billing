...
PaymentProcessorsRouter.prototype.handleCoinpaymentsAddress = function(req, res, next) {
  const PaymentProcessor = this.models.PaymentProcessor;
  ...
  PaymentProcessor
    ...
    .catch((err) => {
      logger.error('Error finding payment processor: ', err);
      return res.status(500).send(errors.InternalError());
    });
};
...
PaymentProcessorsRouter.prototype._definitions = function() {
  return [
    ...
    ['POST', '/pp/method/coinpayments',
      this.authenticate,
      this.handleCoinpaymentsAddress
    ]
  ];
};
